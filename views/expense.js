    ...
    downloadreportbtn.addEventListener('click', async (e) => {
        try {
            ...
            const res = await axios.get(`http://3.111.32.89/download`, { headers: { "Authorization": token } });
            console.log('response when clicked download file', res);
            //when response status is 200 then create element a and download file
            if (res.status === 200) {
                const resp = await axios.post(`http://3.111.32.89/postLink`, { link: res.data.fileUrl }, { headers: { "Authorization": token } });
                console.log(resp);
    ...
    async function addtodb(obj) {
        try {
            const token = localStorage.getItem('token');
            const res = await axios.post(`http://3.111.32.89/addData`, obj, { headers: { "Authorization": token } });
    ...
    function showonscreen(obj) {
        ...
        del.addEventListener('click', async (e) => {
            ...
            const token = localStorage.getItem('token');
            const data = await axios.post(`http://3.111.32.89/delete`, obj, { headers: { "Authorization": token } });
            console.log(data);
        })
    }
    ...
    window.addEventListener('DOMContentLoaded', async () => {
        try {
            ...
            const resp = await axios.get(`http://3.111.32.89/checkpremiumuser`, { headers: { "Authorization": token } })
            ...
            <table border="2" width="100%" id="leaderboardelement">
                ...
                <tbody id="tbodyleaderboard">
                    ...
                    const res = await axios.get(`http://3.111.32.89/premium/leaderboard`);
            ...
            //if user is NOT PREMIUM user
                ...
                buypremium.addEventListener('click', async (e) => {
                    ...
                    e.preventDefault();
                    const token = localStorage.getItem('token');
                    const res = await axios.get(`http://3.111.32.89/purchase/premiummembership`, { headers: { "Authorization": token } });
                    console.log('response after getting /purchase/premiummemb.', res);
                    var option = {
                        "key": res.data.key_id,
                        "orderid": res.data.order.id,
                        "handler": async function (response) {
                            await axios.post(`http://3.111.32.89/purchase/updatetransactionstatus`, {
                    ...
                    rzp1.on('payment.failed', function (response) {
                        ...
                        axios.post(`http://3.111.32.89/purchase/updatetxnfailed`, {
                            order_id: option.orderid,
            ...
            const page = 1;
            const res = await axios.post(`http://3.111.32.89/getPageData?page=${page}`, { expensePerPage: ePP }, { headers: { "Authorization": token } });
    ...
    async function downloadolderversionfile() {
        try {
            ...
            const res = await axios.get(`http://3.111.32.89/getFileLinks`, { headers: { "Authorization": token } });
            console.log(res.data);
